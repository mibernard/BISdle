...
export default function Home() {
  ...
  const [currentIndex, setCurrentIndex] =
    useState(null);
  const [guessedChampions, setGuessedChampions] =
    useState([]);
  ...
  const handleGuess = () => {
    ...
    const isCorrect =
      input.toLowerCase() ===
      unitName.toLowerCase().split("_")[1];
    console.log(
      "input:",
      input.toLowerCase(),
      "answer:",
  ...
  const handleAnswer = () => {
    ...
    setFeedback(
      `The answer is ${unitName.split("_")[1]}`
    );
  };
  ...
  const getTopItemsForChampion = (index) => {
    ...
    if (
      ...
      console.log(
        `No items data available for ${championName}`
      );
      // handleNewItems();
    ...
    const topThreeItems = sortedItems
      .slice(0, 3)
      .map((item) => item[0]);
    ...
    console.log(
      `Top 3 items for ${championName}:`,
      topThreeItems.map(getImageUrl)
    );
    return { championName, topThreeItems };
  };
  ...
  function getImageUrl(itemName) {
    ...
    console.log(
      "kebab:",
      pascalToKebab(itemName)
    );

    return `${baseUrl}${pascalToKebab(
      itemName
    )}.png`;
  }
  ...
  return (
    <center class="back">
      <div>
        ...
        <h1>{currentMode}</h1>
        <p>
          (not rly bis but most frequent items
          placed on this unit in sojus last 10
          games)
        </p>
